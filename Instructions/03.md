...
1. Click on the **Deploy to Azure** button, If prompted **Signin** with the lab credentials. Ensure to deploy the ARM template in the lab account provided. 
   ...
   >**IMPORTANT**: If a new window is opened outside the virtual machine, copy the link provided below then open the copied link in the browser of the virtual machine to deploy the ARM Template. This link will deep link into the Azure Portal, passing in the ARM Template for deploying the resources.
...
5. In the **Platform DevOps and Automation** section, select **No** option for Deploy integrated CICD pipeline?.

   ![](images/Ex3-dep4.png) 

6. In the **Landing zone configuration** section, your tenant will be enabled to host workload Landing Zones. Configure the section as illustrated in the following screenshot this will deploy the chosen policy assignment at the Landing Zone scope. Select the Landing zone subscription that is **L3 - ES Landing Zone Sub -Suffix**
...
12. In the **WT** Management Group, click on **details**
   
    ![](images/ex3-mgdetails.png) 

13. Next, in the **WT** Management Group, click on **Policy** 

    ![](images/ex3-WT-policy.png) 

14. In the **Policy | Definitions** blade, select the scope **WT** and under **Type** select **Custom**, you will see a list of Initiative and policy definitions as shown below which are assigned at the management group level.
...
16. On the **Resource Groups** blade, Select the subscription **L3- ES Management Sub - Suffix** and select the resource group **WT-mgmt**.
     
    ![](images/ex3rg.png)

17. On the **WT-mgmt** page, review the resources deployed by the ARM template
...
2. On the **Key vaults** page click on **+Create** .
...
3. **Create a key vault** by providing the following details:
   ...
   - Provide the keyvault name : **kvDeploymentID**
     >Get the **DeploymentID** value from the Environment details tab , Suppose 257725 is the DeploymentID , the keyvaultname should be like kv257725.
...
4. Once the deployment is completed, monitor the **Activity Log** to see the policy effect. You should see **deployIfNotExists** Policy action and examine the JSON payload.

   To do this, navigate to **kvmgmt** resource group and select **Activity log** from the left hand menu, you should see that **deployIfNotExists** policy is enforced and it is the beginning of policy assignment. **Refresh** the activity log page until you see the **deployIfNotExists** policy(This may take upto 2 minutes to reflect in the activity log)
...
5. Post a successful deployment of the Key vault, you should start to see (approximately 6-10 minutes) another deployment of the resource appearing in the resource group **kvmgmt** under **Deployments** section.
 ...
 >**deployIfNotExists** policy effect takes place and enables the Log analytics workspace for the key vault.
