...
1. In GitHub repo, navigate to the following path `eslz/root/tenant root group (xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx)/eslz (eslz)/eslz-landingzones (eslz-landingzones)/eslz-corp (eslz-landingzones-corp)/l2 - identity sub - xxxx (xxxx-xxxx-xxxx-xxxx-xxxx)/  (1)`. Click on **Add file (2)** and **Create new file**. 
