...
1. Click on the **Deploy to Azure** button.
  ...
  >If a new window is opened outside the virtual machine, select the **Deploy to Azure** button right click & copy the link then open the copied link in browser of virtual machine to deploy the ARM Template. This link will deep link into the Azure Portal, passing in the ARM Template for deploying the resources.
...
13. Also review the resources deployed by the ARM template.
...
2. In the Azure portal , type **keyvault** at the search bar and select **Keyvaults** under services.
...
6. Post a successful deployment of the KeyVault, you should start to see (approx. 6-10 minutes) another deployment of the resource appearing in the resource group **kvmgmt**
   
   ![](images/ex3t2-4.png)
   
   >deployIfNotExists policy evaluates to determine if Log analytics workspace is enabled for keyvault. If not, then a deployment to enable is executed by enforcing the policy.
  
7. Navigate to key vault you just created and review the **Diagnostics settings**, verify  if it is connected to your **Log Analytics workspace**.

   ![](images/ex3t2-5.png)
   
8. Now, on the keyvault page click on **Insights** under **Monitoring** and explore the Azure Monitor Insights view

   ![](images/ex3t2-6.png)
  
9. Navigate to Azure **Policy** and select the **management group** hosting the management subscription, and review compliance state of the policies.
...
2. Under **Policy** blade select **Definitions** under the Authoring section and select the **Online management group** as scope.
   ...
   >Note: When a Key Vault is created with out soft delete enabled then this policy Append-KV-SoftDelete is enforced and will enable the soft deletion. 
...
4. **Assign** the policy at the **Online** management group scope, and provide the assignment a description as **This policy will ensure all key vaults are being created with soft deletion enabled**.
