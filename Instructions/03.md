...
5. Once the deployment is started, monitor the **Activity Log** to see the policy effect. You should see **deployIfNotExists** Policy action and examine the JSON payload.

   To do this, navigate to **kvmgmt** resource group and select **Activity log** from the left hand menu, you should see that **deployIfNotExists** policy is enforced and it is the beginning of policy assignment.
...
3. Review the policy definition **Append-KV-SoftDelete** and check what the policy will do on Key vault resources. Once reviewed, click on **Assign** to assign the policy to a particular scope.
