...
3. On the **Key vaults** page click on **+Create** .
...
5. Once the deployment is completed, monitor the **Activity Log** to see the policy effect. You should see **deployIfNotExists** Policy action and examine the JSON payload.
...
6. Post a successful deployment of the Key vault, you should start to see (approximately 6-10 minutes) another deployment of the resource appearing in the resource group **kvmgmt** under **Deployments** section.
...
9. Navigate to Azure **Policy**, select **Compliance** from the left hand menu and change the **scope** to **WT-management**. Review the compliance state of the assigned policies.

   ![](images/ex3t2-10.png) 
   
10. Next, you will check the policies that are **non-compliant** in your environment, and why.

11. Click on the first policy definition that is non-compliant.

    ![](images/ex3-task2-10.png)
    
12. Then click on the policy that is non-compliant from the **Policies** tab
    
    ![](images/ex3-task2-11.png) 
     
13. Next, under the **Resource compliance** tab, right click and select **Compliance reason** to know why the policy is non-compliant
