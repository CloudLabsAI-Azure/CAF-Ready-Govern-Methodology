...
5. Remove the existing JSON code( how to remove this?) for **Deploy Vnet Landing zone** then click on folder symbol under Import template and select the file **vnetdmz-deploy.json** located in **C:\LabFiles** (should this be C:\ or a cloned path?) which deploys the **virtual network** with DMZ architecture.
...
8. Click on **Publish blueprint**. (should the name of step to deploy VNet be "Deploy DMZ VNet"? VNet Landing Zone was deployed and removed earlier.)
...
18. Select the virtual network and under the **settings** select **Subnets** option to see the created subnets with Virtual network.
   ...
   - And **Azure Bastion** allows you to log into VMs in the virtual network through SSH or remote desktop protocol (RDP) without exposing the VMs directly to the internet. Use Bastion to manage the VMs in the virtual network.
     ...
     In this exercise, you created a blueprint based on CAF Migration Landing Zone and explored the capabilities it provides. This implementation option does not implement any governance tooling. It is assumed that this landing zone is being used for limited production deployment to initiate learning, iteration, and development of the overall operating model in parallel to these early-stage migration efforts. To accelerate the parallel development of governance disciplines, you will be deploying the **CAF Foundation blueprint** in the next exercise (next section appears to be AzOps for Enterprise-scale with no reference to another blueprint. am i missing something?).
     
     Click on **Next** (missing link?) to proceed with the next exercise.
