...
3. Now on the **Create blueprint** page, under the **Choose a blueprint sample** header, type **CAF** at the search bar and select the predefined blueprint sample **CAF Foundation Zone**. You will be redirected to **Create blueprint** page.

   ![](images/EX2-CAFFoundationZone.png) 
 
4. On the **Create blueprint** page create the blueprint with the following characteristics:
   - Name : `L2-CAFFoundationzone`
...
8. Navigate to the **Blueprints** blade then under the **Blueprint definitions** section select the scope **L2-Governance Sub-Suffix** and click on the blueprint definition **L2-CAFFoundationzone** .
      
9. Click on **Publish blueprint**.
   
   ![](images/ex2-04-publishbp.png) 
   
   >Note: When a blueprint is first created, it's considered to be in Draft mode. When it's ready to be assigned, it needs to be Published.
...
12. Navigate back to the **Blueprints** blade then under the **Blueprint definitions** section select the published blueprint definition **L2-CAFFoundationzone**.
...
14. On the **Assign blueprint** page, Provide the parameter values for the blueprint assignment:
     ...
     **Basics**
     ...
     - **Subscriptions**: Choose the subscription `L2-Governance Sub-Suffix` where your copy of the blueprint sample is saved to.
      
     - **Assignment name**: The name is pre-populated for you based on the name of the blueprint **Assignment-L2-CAFFoundation**
     ...
     **Artifact parameters**
      ...
      - Allowed virtual machine SKUs : Leave the default values here,you can select the small sized vm sku size inroder to reduce the cost 
...
16. To check on the status of deployment, click on **notifications** icon and click on the **Blueprint assignment succeeded**
...
17. Observe the resources which are being deployed by the blueprint.
    ...
    >You can also check the deployment status by opening the blueprint assignment **Assignment-L2-CAFFoundation** from the **Blueprints** page and **Assigned blueprints** section, ensure you select the right scope **L2-Governance Sub-Suffix**.
...
19. In the upper left corner of the portal window, to open the Resource Groups menu, click the toggle menu icon and then click on **Resource groups**.
 ...
 20. On the **Resource Groups** blade filter the resource groups by Subscription**L2-Governance Sub-Suffix** and observe that four resource groups named **TTDeploymentIDApplication-rg,TTDeploymentIDVnet-rg, TTDeploymentIDIdentity-rg and TTDeploymentIDSharedSvcs-rg** are created .
 
     >**TTDeploymentIDApplication-rg, TTDeploymentIDVnet-rg and TTDeploymentIDIdentity-rg** are empty resource groups.
     
     ![](images/ex2-rgs.png)
       
 21. Select the resource group **TTDeploymentIDSharedSvcs-rg** and review the deployed resources.
 ...
 22. Now, type **Policy** at the search bar and select **Policy** under services.

     ![](images/ex2-policysearch.png)
     
 23. Review the policies are assigned at the subscription **L2-Governance Sub-Suffix** by this blueprint.
...
1.  Navigate to **Blueprints** page under the **Blueprint definitions** section select **L2-CAFFoundationzone** Click on **Edit blueprint**.
...
2. On the **Edit blueprint** click on **Next:Artifacts>>** and **explore the artifacts** included by default with the **CAF Foundation Zone** blueprint sample.
   ...
   **The blueprint also defines and assigns the following Azure Policy definitions:**
   ...
   - Resource Types that you do not want to allow in your environment : You can select only the resource types which are needed for the environment and restirct the other resources by enforcing this policy.
...
In this task you will be updating the blueprint by assigning the policy definitions inorder to increase the security level.

1. On the **Edit blueprint** page of **L2-CAFFoundationzone** Click on **+Add Artifact** and select the **Policy definition** as the artifact type.
   
   ![](images/ex2-10-addartifact.png) 

2. **Search** for the following policy definition  
   
   -Storage accounts should restrict network access == Audit unrestricted network access to storage accounts
   
    Select the policy definition and click on **Add**.
    
    ![](images/ex2-11-search.png) 
   
 3. Similarly search the remaining policy definitions and click on **Add**
...
5. Now navigate to **Blueprints** page and under the **Blueprint definitions** section click on the edited blueprint definition **L2-CAFFoundationzone**.
...
7. Under **Blueprints->Assigned blueprints**, choose the scope **L2-Governance Sub-Suffix** and then select the assignment **Assignment-L2-CAFFoundation** and click on **Update assignment** to update the assignment with latest version of blueprint.

    ![](images/ex2-12-updateassignment.png) 
   
8. On the **Assign blueprint** provide the parameter values for the to update the blueprint assignment:
   ...
   - **Under Artifact parameters**
    ...
    - Resource Types that you do not want to allow in your environment: Search for **expressroute** and select all resources of expressroute.
        
      ![](images/ex2-07-assignbp.png)
        
    - **Leave all the other options to default prepopulated values.**

9. Once all parameters have been entered, Click on **Assign** at the bottom of the page.

10. The blueprint assignment is created and artifact deployment begins , the deployment takes around **five** minutes. 

11. To check on the status of deployment, open the blueprint assignment.

12. After the assignment is succeeded, In the upper left corner of the portal window to open the Resource Groups menu, click the toggle menu icon and then click on **Resource groups**.
...
13. Select the resource group **TTDeploymentIDSharedSvcs-rg** and under the **settings** section select **Locks** to see that resource locks are applied for each resource in the resource group.

14. Now, type **Policy** at the search bar and select **Policy** under services.

    ![](images/ex2-policysearch.png)
       
15. Review all the policies assigned by the blueprint by selecting the scope `L2-Governance Sub-Suffix`

    ![](images/ex2-policies.png)
...
2. **Append CostCenter TAG & its value from the Resource Group**
   ...
   ![](images/ex2-16-costrgresources.png)
   
   ![](images/ex2-15-costrgresource.png)
