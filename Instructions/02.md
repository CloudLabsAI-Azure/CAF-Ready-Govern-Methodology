...
5. Remove the existing JSON code for **Deploy Vnet Landing zone** then click on folder symbol under Import template and select the file **vnetdmz-deploy.json** located in **C:\LabFiles** (should this be C:\ or a cloned path?) which deploys the **virtual network** with DMZ architecture.
