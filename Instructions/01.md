...
10. Enter the version as **v1.0** and click on **Publish** to publish the blueprint.
 ...
 14. Now on the **Assign blueprint** page , provide the parameter values for the blueprint assignment:
     ...
     **Artifact parameters**
      ...
      - **Deploy log analytics:** `30`
        >Provide the value for Number of days data will be retained in Log Analytics.
...
16. To check on the status of deployment, open the blueprint assignment **Assignment-L1-Migrationlandingzone** from the **Blueprints** pane and**Assigned blueprints** section.
...
19. As you can see three resource groups named **-Migrate-rg,-SharedSvcs-rg,-VNet-rg** are deployed by the blueprint.
...
In this architecture, **Azure Firewall** is a managed firewall as a service, the Firewall instance is placed in its own subnet and it is deployed in the virtual network to control traffic between the gateway's subnet and the subnet in which the application tier runs. In this way Azure Firewall is cost effective because it's used as a shared solution consumed by multiple workloads.
...
- In the **web tier subnet**, Since there is no route for address space of the VNet itself to point to Azure firewall, web tier instances are able to communicate directly to each other, not via Azure Firewall.
  ...
  Azure **Bastion** allows you to log into VMs in the virtual network through SSH or remote desktop protocol (RDP) without exposing the VMs directly to the internet. Use Bastion to manage the VMs in the virtual network.
