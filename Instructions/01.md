...
- [Task 1: Deploy Enterprise-Scale with hub and spoke architecture](#task-1-deploy-enterprise-scale-with-hub-and-spoke-architecture)
   ...
   - [Enterprise-Scale core setup](#enterprise-scale-core-setup)
   - [Platform management, security, and governance](#platform-management-security-and-governance)
   - [Platform DevOps and Automation](#platform-devops-and-automation)
   - [Network topology and connectivity](#network-topology-and-connectivity)
   - [Identity](#identity)
   - [Landing zone configuration](#landing-zone-configuration)
...
1. On the **Azure corp setup** section, provide a Management Group prefix (for this lab, we will use **eslz** as the prefix).
   ...
   - click **Next: Platform management, security, and governance >** (3) to move to the next section.

   ![](images/azurecorpsetup01.png)
   
1. On the **Platform management, security, and governance** blade, you will configure the core components to enable platform monitoring and security. The options you enable will also be enforced using Azure Policy to ensure resources, landing zones, and more are continuously compliant as your deployments scales and grows. To enable this, you must provide a dedicated (empty) subscription that will be used to host the requisite infrastructure. Use following configuration:
   - Select **Yes(recommended)** (1) option for **Deploy Azure Security Center and enable security monitoring for your platform and resources**.
...
1. Generate GitHub **personal access (PA)** token for authenticate to your GitHub account programatically by deployment, this token will be used by the deployment to create a new repository and discover the Enterprise-Scale deployment ARM templates and merge them into your main branch.
...
1. On **Platform DevOps and automation** section, make sure for `Deploy integrated CICD pipeline` **Yes** option is selected, then
...
1. On **Register an application** blade, enter following details:
   ...
   - Now, click on **Register** button to register application and move to the next step.

   ![](images/registerapplication.png)

1. After registering application, application blade will open automatically. In that click on **+ New Client secret** (2) under **Certificates and secrets** (1). 

   ![](images/appcertificates.png)
   
1. Now, on **Add a client Secret** blade, enter following:
...
1. Once you have added the service principal secret.  Under Client Secrets look for the secret `eslzsp-secret` which you have added in previous steps and **copy** the **secret value** using the copy icon (2) and make sure to note down the “Value” of the new client secret somewhere for next step. Now, click on **Azure landing zone accelerator** (3) to continue with the template deployment.
   
    ![](images/copy-secret.png)
    
1. **Platform DevOps and automation** section, enter the service principal secret value in **Password** (1) field which you have copied and then click on **Next: Network topology and connectivity** (2). 

    ![](images/paste-secret.png)
 
1. For **Network toplogy and connectivity** section, you will configure the core networking platform resources, such as hub virtual network, gateways (VPN and/or ExpressRoute), Azure Firewall, DDoS Protection Standard and Azure Private DNS Zones for Azure PaaS services. Select following:
...
1. On **Landing zone configuration** section,
   - Select **Yes (recommended)** (1) for Connect corp landing zones to connectivity hub (optional)?,
...
1. On **Review + Create** page, required permission and configuration will be validated before you can click deploy. Once it has been validated successfully, you can click **Create** to begin the deployment.
...
1. Once the template deployment is completed you will see **Your deployment is complete** message on deployment page. It will take almost 90 Minutes to succeed.
