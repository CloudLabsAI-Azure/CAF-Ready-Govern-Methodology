...
>Note: **L1-MigrationLZ Sub-Suffix** subscription is used to perform this exercise. Suffix is the 4 digit XXXX value of the tenant , for example if the username is odl_user_238206@azurehol**1505**.onmicrosoft.com, 1505 is the Suffix value.
...
5. Next, set the **Definition Location** value to a subscription named **L1-MigrationLZ Sub-Suffix** and then click on **Select** and return back to the Create blueprint page.
...
8. Navigate to the **Blueprints** blade then under the **Blueprint definitions** section select the blueprint definition **L1-MigrationLandingzone**.
   ...
   >Change the scope to the subscription **L1-MigrationLZ Sub-Suffix** if it is set to different subscription.
   
9. Click on **Publish blueprint**.

   ![](images/ex1-06-publishbp.png) 
   
   >When a blueprint is first created, it's considered to be in Draft mode. When it's ready to be assigned, it needs to be Published , so the next step is to **publish the blueprint**.
...
20. On the **Resource Groups** blade filter the resource groups by Subscription **L1-MigrationLZ Sub-Suffix** and verify that three resource groups named **-Migrate-rg, -SharedSvcs-rg and -VNet-rg** are created by the blueprint.

    ![](images/ex1-resourcegroupsnames.png)
    
21. Review the deployed resources in each resource groups.

22. Under the **Resource groups** blade Click on **Migrate-rg** and then Click on **Show Hidden types** to see the resources created in this resource group.
...
23. Now return to the **Resource groups** blade and click on **SharedSvcs-rg** and review the deployed resources.
...
24. Again return to the **Resource groups** blade and click on **VNet-rg** to check for the resources created in this resource group
 ...
 25. Click on **Virtual network** in the resource group and then select **Subnets** under Settings section and look at the subnets deployed.
   
     ![](images/ex1-task1-subnets.png)
     
     A virtual network will be created with subnets for gateway, firewall, jump box, and core subnet and a subnetwork or subnet is a logical subdivision of an IP network.
...
In this task you will update the VNET Landing zone to deploy a virtual network to add an additional layer of security which uses the **DMZ architecture**(Demilitarized Zone). The architecture implements a DMZ, also called a **perimeter network**, between the on-premises network and an Azure virtual network.

The purpose of a DMZ is to add an additional layer of security to an organization's local area network,an external network node can access only what is exposed in the DMZ, while the rest of the organization's network is firewalled
...
1. Navigate to **Blueprints->Assigned blueprints** and select **Assignment-L1-Migrationlandingzone** then click on **Unassign blueprint** to unassign the previously assigned blueprint.
   ...
   >select the scope **L1-MigrationLZ Sub-Suffix** if it is set to different subscription.
...
9. Navigate to **Blueprints** pane and under the **Blueprint definitions** section click on the edited blueprint definition **L1-Migrationlandingzone**. 
