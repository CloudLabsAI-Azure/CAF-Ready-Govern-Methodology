...
7. On the **Enable Inventory** page choose the subscription **Sub 05-Suffix** and resource group **CAF**, then select the virtual machine **labvm-DeploymentID**. Click on **Enable** to add the virtual machine to enable Change Tracking and Inventory.
...
2. On the **Automation account| Update Management** choose the Log Analytics workspace **logan-DeploymentID** and Automation account **autoac-DeploymentID** then click on **Enable** to enable the log analytics workspace for Update management solution.
...
13. Now on the **Automation account| Logs** blade, click on **Select a scope** and set the scope to the virtual machine **labvm-DeploymentID** then click on **Apply**.
...
14. In Log Analytics, paste the following query and click on **Run** to search for changes to the hosts file:
    ...
    >IMPORTANT: The query will return **No results found** as the change tracking process takes 45 minutes to show the required output. Please continue with the lab, you will review the output once again at the end of the lab.
...
1. Navigate to **Automation account** blade and under the **Update management** section, select **Update management**.
...
#### Task 4: Review the result of Change Tracking process in log analytics workspace.
  
  In this task, you will once again review the results of subtask3 as Change Tracking and Inventory checks for the changes in a windows file for every 30 minutes.
...
2. Now on the **Automation account| Logs** blade, click on **Select a scope** and set the scope to the virtual machine **labvm-DeploymentID** then click on **Apply**.
  
    ![](images/ex4-logs-vm.png)

3. In Log Analytics, paste the following query and click on **Run** to search for changes to the hosts file:
    ```
    ConfigurationChange | where FieldsChanged contains "FileContentChecksum" and FileSystemPath contains "hosts"
    ```
    ![](images/ex4-query.png)
    ...
    >IMPORTANT: If it returns the output as **No results found** again, there might be delay in the process.
