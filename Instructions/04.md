...
5. Now on the virtual machine navigate to  the path **C:\windows\system32\drivers\etc\hostsC:\windows\system32\drivers\etc\hosts** and open with **Notepad** to edit the file.
   
   ![](images/ex4-path.png)
   
6. Edit the file **hosts** to check how **Change Tracking** tracks the file.
   
   ![](images/ex4-edithosts.png)

7. Now return back to the **autoac-DeploymentID**  **Automation account** blade 

8. From the **autoac-DeploymentID**  **Automation account** page scroll down to **Monitoring** section and select **Logs**.
  
   ![](images/ ex4-logs.png)
   
9. Now on the **Automation account| Logs** blade click on **Select a scope** and set the scope to the virtual machine **labvm-DeploymentID** and click on **Apply**.
  
  ![](images/ex4-logs-vm.png)

10. In Log Analytics, run the following query to search for changes to the hosts file:
    ```
    ConfigurationChange | where FieldsChanged contains "FileContentChecksum" and FileSystemPath contains "hosts"
    ```
   ![](images/ex4-query.png)
   
11. Now next step is to enable tracking for file content changes.
...
12. Navigate to **autoac-DeploymentID** Automation account blade, select **Change tracking** under **Configuration Management**.

13. Select **Edit Settings** 
   
   ![](images/ex4-6-editsettings.png)

14. On the **Workspace Configuration** page, select **File Content** and click on **Link** to link a storage account where the changes done to a file is saved.
