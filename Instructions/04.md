...
6. On the **Enable Inventory** page choose the subscription **Sub 05** and resource group **CAF** then select the virtual machine **labvm-DeploymentID** and click on **Enable** to add the virtual machine to enable Change Tracking and Inventory.
...
4. On the **Enable Update Management** page choose the subscription **Sub 05** and resource group **CAF** then select the virtual machine **labvm-DeploymentID** and click on **Enable** enable Update Management.
...
3. On the **Workspace Configuration** page, select **Windows Files** tab and then click **+ Add** to add a file to be tracked.
...
7. Next on the **Add Content Location for Change Tracking** blade Select the subscription **Sub 05-DeploymentID** ,select the storage account **StorageDeploymentID** for storing the file contents from the drop down menu.
...
10. Edit the file **hosts** by editing the values inorder to check how **Change Tracking** solution tracks the file.
   
    ![](images/ex4-edithosts.png)
   
    >For example if the value is 123 ,append any other number to it.

11. Now return back to azure portal and go to the **autoac-DeploymentID** **Automation account** blade 
...
14. In Log Analytics, run the following query to search for changes to the hosts file:
    ```
...
11. On the **Policy** page select the scope as **sub 05-DeploymentID** and verify if the policies applied in the previous steps are successfully assigned.
