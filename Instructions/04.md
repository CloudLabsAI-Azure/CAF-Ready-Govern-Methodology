...
4. On the **Automation account| Inventory** page ensure the Log Analytics workspace **logan-DeploymentID** and Automation account **autoac-DeploymentID** are already selected and click on **Enable** to enable the log analytics workspace for Inventory solution.
...
7. On the **Enable Inventory** page ensure the subscription **Sub 05-Suffix** and resource group **CAF** are selected by default, then select the virtual machine **labvm-DeploymentID**. Click on **Enable** to add the virtual machine to enable Change Tracking and Inventory.
...
8. After adding the Virtual machine to Inventory, it should show under the **Machines** tab.
   ...
   >It will take upto 45 minutes for the virtual machine to reflect under the **Machines** tab, You can continue with the lab.
...
1. Navigate to **autoac-DeploymentID**  **Automation account** blade and under the **Update management** section, select **Update management**.
 
   ![](images/ex4-updatemanagement.png)

2. On the **Automation account| Update Management** ensure the Log Analytics workspace **logan-DeploymentID** and Automation account **autoac-DeploymentID** are selected then click on **Enable** to enable the log analytics workspace for Update management solution. **Refresh** the page after the deployment is completed.
...
4. On the **Enable Update Management** page ensure the subscription **Sub 05-Suffix** and resource group **CAF** are already selected then select the virtual machine **labvm-DeploymentID** and click on **Enable** enable Update Management.
...
7. Next on the **Add Content-Location for Change Tracking** blade, select the subscription **Sub 05-Suffix** then select the storage account **StorageDeploymentID** for storing the file contents from the drop-down menu.
...
10. Edit the file **hosts**, by editing the values in order to check how **Change Tracking** solution tracks the file.Ensure to **Save** the file after editing.
...
4. In **advanced settings**, select the subscription **Sub 05-Suffix** and region **CentralUS** 
   ...
   Then select **CAF** resource group and choose the **existing storage account** **storageDeploymentID** create a file share fs, and click on **Create Storage**.
...
12. On the **Policy** page, select the scope to **sub 05-Suffix** subscription and verify if the policies applied in the previous steps are successfully assigned.
...
2. From the **Automation account| Update Management** page select **+Schedule update deployment** option. You will be redirected to **New update deployment** page.
 ...
 3. On the **New update deployment** page. Provide the following details and click on **Create**.
    ...
    - Machines to update: Under **Type**, select **Machines** from the dropdown. Under **Machine name** select **labvm-DeploymentID**
   ...
   >If you don't see any machine, note that for machines configured recently it can take few minutes to appear in the list.
...
1. In the Azure Portal, from the **autoac-DeploymentID**  **Automation account** page, scroll down to **Monitoring** section and select **Logs**.
  
    ![](images/ex4-logs.png)
    
2. Click on **Get Started** and close the window if a queries window opens.
   
3. Now on the **Automation account| Logs** blade, click on **Select a scope** and set the scope to the virtual machine **labvm-DeploymentID** then click on **Apply**.
...
4. In Log Analytics, paste the following query and click on **Run** to search for changes to the hosts file:
    ...
    >IMPORTANT: If it returned the output as **No results found**, then the Change Tracking and Inventory checks have not happened yet or there might be a delay in the file tracking process.
 
 5. You can also check for the changes made by navigating to **autoac-DeploymentID** **Automation account** blade, select **Change tracking** under **Configuration Management** and **Changes** tab at the bottom left.
