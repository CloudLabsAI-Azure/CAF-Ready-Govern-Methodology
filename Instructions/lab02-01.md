...
5. For the 'Scope level' select 'Management Group', in the Management Group dropdown select the target Management Group ```eszl```, in the 'Details' section apply a Service Connection name ```eslz-serviceconnection``  and optional give it a description and click 'Save'.
